...
SolvIER (**Solv**ing **I**ncomplete **E**ntity **R**esolution) is a pipeline that combines transformer-based data imputation with state-of-the-art entity resolution. By imputing missing values before matching, SolvIER consistently improves F1-score over standard ER baselines — with up to **+17.4% relative improvement** on real-world benchmarks at high missingness rates.

## Overview

Entity Resolution (ER) — the task of identifying records that refer to the same real-world entity — is significantly challenged by missing attribute values. 
